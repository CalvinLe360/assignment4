...
public class CouncilMember implements Runnable{
    ...
    public static void main(String args[]){
	    ...
	    for(int i = 1; i <= 100; i++) {
		CouncilMember members = new CouncilMember("Immediate", serverSocketInfo); 
	    ...
	    for(int i = 0; i < 20; i++) {
		value += 1;
		memberList.get(i).setChosenValue(Integer.toString(value));
		memberList.get(i).propose();
	    }
		...
		PrintWriter printWriter = new PrintWriter(fileWriter); 
		printWriter.append("Run Time: " + Long.toString(total_time)); 
		printWriter.append("\n");
		printWriter.close(); 
	    }
	    catch(Exception e){
		...
		memberList.get(k).shutDownServer();
	    }
        }
